...
class SchedulerJob : public BATLedgerJob<bool> {
 ...
 private:
  ...
  void OnNextTimeRead(base::Time time) {
    context()
        .Get<DelayGenerator>()
        .Delay(FROM_HERE, time - base::Time::Now())
        .Then(ContinueWith(this, &SchedulerJob::OnDelayElapsed));
  }

  void OnDelayElapsed(base::TimeDelta) {
  ...
  void OnDataReady(std::tuple<std::vector<RecurringContribution>,
    ...
    context()
        .Get<JobStore>()
        .StartJobWithState<ContributionJob>(ScheduledContributionState{
            .recurring = std::move(recurring), .activity = std::move(activity)})
